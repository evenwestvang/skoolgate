...
.plan:
  * refactor js – i have more javascript fu now. thank you shiny books.
  * add new test data
    - rework models
    - rework statistics
    - refactor display code. 'tis ugly
    - rework display code
  * add high quality numbers for student count & addresses
  * run geocoding batch
  * client must tell server which markers it has to avoid lotsa retransmission

TODO:
  * aggregate results by test type and year for filtering
  * ranked static stats pages: counties / municipalities w/ map links
  * windowed autopanning google streetview panos on #about page. mmmm. tasty.
